...
const domEvents = (uid) => {
  document.querySelector('#main-container').addEventListener('click', (e) => {
    ...
    if (e.target.id.includes('edit-book-btn')) { // if you click edit book button
      ...
      getSingleBook(firebaseKey).then((bookObj) => addBookForm(uid, bookObj)); // get single book from firebase key then add bookobj to books
    }
    ...
    // ADD CLICK EVENT FOR DELETING AN AUTHOR - DELETE BOOKS RELATED TO AUTHOR TOO
    if (e.target.id.includes('delete-author-btn')) {
      // eslint-disable-next-line no-alert
      if (window.confirm('Want to delete?')) {
        const [, firebaseKey] = e.target.id.split('--');
        deleteAuthorBooks(firebaseKey, uid).then(showAuthors); // shorthand for following line
